...
f1 = open("../datasets/cresci-2017/social_spambots_2.csv/social_spambots_2.json", "r", encoding="cp850")
...
data_normal_end = {'users': []}
data_anomaly_end = {'users': []}

for user in data_normal['users']:
    count += 1
    if count % 4 == 0:
        data_normal_end['users'].append(user)
...
for user in data_anomaly['users']:
    count += 1
    if count % 4 == 0:
        data_anomaly_end['users'].append(user)
...
def f(X):
    ...
    os.environ['PAMP_THRESHOLD_TIME_ENTROPY'] = str(
        float(os.environ['SS_THRESHOLD_TIME_ENTROPY']) - float("0." + str(int(X[45])) + str(int(X[46]))))
    ...
    if (float(os.environ['SS_THRESHOLD_AVG_TWEET_SIMILARITY']) + float("0." + str(int(X[49])) + str(int(X[50])))) > 1:
        ...
        os.environ['PAMP_THRESHOLD_AVG_TWEET_SIMILARITY'] = str(
            (float(os.environ['SS_THRESHOLD_AVG_TWEET_SIMILARITY']) + float("0." + str(int(X[49])) + str(int(X[50])))))
    ...
    if (float(os.environ['SS_THRESHOLD_BASIC_RATIO']) + float("0." + str(int(X[53])) + str(int(X[54])))) > 1:
        ...
        os.environ['DS_THRESHOLD_BASIC_RATIO'] = str(
            (float(os.environ['SS_THRESHOLD_BASIC_RATIO']) + float("0." + str(int(X[53])) + str(int(X[54])))))

    os.environ['PAMP_INTERVAL_AVG_TWEET_SIMILARITY'] = "0." + str(int(0)) + str(int(X[56]))
    ...
    os.environ['PAMP_INTERVAL_TIME_ENTROPY'] = "0." + str(int(0)) + str(int(X[62]))
    ...
    result = dca17(data_normal_end, "Normal", 1, data_anomaly_end, "Anomaly", 1)
...
varbound = np.concatenate((np.full((28, 2), [1, 10]), np.full((9, 2), [5, 40]), np.full((1, 2), [0, 60]),
                           ...
                           np.full((1, 2), [3, 5]), np.full((4, 2), [0, 9]), np.full((30, 2), [0, 9]),
                           np.full((1, 2), [2, 8])),
                          axis=0)

vartype = np.concatenate((np.full((42, 1), 'int'), np.full((36, 1), 'int')),
                         axis=0)
...
model = ga(function=f, dimension=78, variable_type_mixed=vartype, variable_boundaries=varbound, function_timeout=36000)
