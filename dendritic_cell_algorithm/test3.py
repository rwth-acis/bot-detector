...
f1 = open("../datasets/cresci-2017/genuine_accounts.csv/result-1.json", "r", encoding="cp850")

# Reading from file
data1 = json.loads(f1.read())
f2 = open("../datasets/cresci-2017/social_spambots_1.csv/result.json", "r", encoding="cp850")
...
def f(X):
    ...
    os.environ['PAMP_INTERVAL_SMALL_INTERVAL'] = str(round(X[47], 2))
    os.environ['PAMP_INTERVAL_TIME_ENTROPY'] = str(round(X[48], 2))
    os.environ['SS_INTERVAL_TIME_ENTROPY'] = str(round(X[49], 2))
    ...
    os.environ['DS_INTERVAL_HASHTAG_TWEET_RATIO'] = str(round(X[54], 2))
    os.environ['SS_THRESHOLD_HASHTAG_TWEET_RATIO'] = str(round(X[55], 2))
    os.environ['SS_INTERVAL_HASHTAG_TWEET_RATIO'] = str(round(X[56], 2))
    os.environ['DS_THRESHOLD_AVERAGE_FAVORITE_COUNT'] = str(round(X[57], 2))
    os.environ['DS_INTERVAL_AVERAGE_FAVORITE_COUNT'] = str(round(X[58], 2))

    print(str(os.environ)[2912:])
    result = dca17(data1, "Normal", 50, data2, "Anomaly", 50)
...
model = ga(function=f, dimension=59, variable_type_mixed=vartype, variable_boundaries=varbound, function_timeout=36000)
